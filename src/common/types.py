...
class MissionInfo:
    ...
    # Operator-provided fields (DJI EXIF/XMP has none of these) -- filled in
    # from a session_metadata.yaml sidecar by capture/image_catalog.py, never
    # guessed. See CheckCrack_Operator_Camera_Capture_Settings_Guide.
    capture_session: Optional[str] = None
    capture_distance_m: Optional[float] = None
    distance_measurement_method: Optional[str] = None
    # Same flight_id across a battery swap (1 Facade = 1 Flight still holds);
    # this only records which battery within that flight took the photo.
    battery_sequence: Optional[int] = None
    # Set when this image belongs to a supplemental close-range (5m) session
    # re-shot for a specific crack/zone flagged during the primary (10m) pass
    # -- the crack_id or zone identifier it's following up on.
    precision_zone_of: Optional[str] = None


@dataclass
class OperatorSessionInfo:
    """Fields the operator records in a session_metadata.yaml sidecar, per the
    CheckCrack_Operator_Camera_Capture_Settings_Guide's "운용자 필수 Metadata"
    slide. Pure provenance -- no pipeline logic branches on these today, but
    they're kept traceable per CLAUDE.local.md #39 (Evidence/Provenance)."""

    capture_date: Optional[str] = None
    building_block: Optional[str] = None
    building_side: Optional[str] = None
    image_original_preserved: Optional[bool] = None
    reference_scale_available: Optional[bool] = None
...
class ImageMetadata:
    ...
    operator_session: OperatorSessionInfo = field(default_factory=OperatorSessionInfo)
...
class SourceObservation:
    """One original (pre-stitch) photo a crack is actually visible in, and
    where within that photo -- as opposed to Crack.source_image_ids, which is
    facade-level provenance (every photo used for the whole facade). Computed
    in crack/pipeline.py by intersecting the crack's mosaic-space polygon
    against the stitch stage's persisted seam-ownership map, then mapping
    through that source image's own homography inverse. Sorted by
    owned_pixel_count descending -- source_observations[0] is the best single
    photo to show for this crack."""

    image_id: str
    bbox_px_in_source: tuple  # (x0, y0, x1, y1)
    polygon_px_in_source: "object"  # np.ndarray (N, 2)
...
class Crack:
    ...
    # "정밀점검대상" / "경미" -- set only when max_width_mm is not None (real
    ...
    # where -- list[SourceObservation]. Empty when the stitch that produced
    # this mosaic predates homography/seam-map persistence (older facades),
