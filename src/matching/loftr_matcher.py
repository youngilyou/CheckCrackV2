...
class TimeoutLoFTRMatcher:
    ...
    Confirmed live (2026-08-15/16): LoFTRMatcher.match() can hang 10+ minutes
    on a specific pair while every other pair in the same batch matches in
    ~1-2s -- consistent with CLAUDE.local.md #9's own warning that repetitive
    window patterns ("반복 창문 패턴") cause matching trouble. A same-process
    timeout (e.g. a background thread with future.result(timeout=...)) can't
    actually recover from this: CUDA calls queued from different host threads
    still serialize on the same GPU context, so an abandoned thread's stuck
    kernel keeps blocking every pair after it anyway. Only killing the whole
    process tears down that CUDA context, so a timeout here kills the worker
    process and starts a fresh one -- the timed-out pair is then reported as
