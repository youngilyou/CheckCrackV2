...
def build_owner_map(
    seam_masks: dict[str, np.ndarray], warped: dict[str, WarpedImage], canvas_size: tuple[int, int]
) -> tuple[np.ndarray, list[str]]:
    """Combines per-image local seam_masks into one full-canvas label image
    for persistence (see crack/pipeline.py's source_observations, which needs
    "which source image owns this mosaic pixel" available long after this
    stitch run's own process has exited, since crack detection runs as a
    completely separate later CLI invocation against saved files -- there is
    no in-memory path from here to there).

    Returns (owner_map, ids): owner_map is uint16, 0 = no owner (background/
    never-covered pixel), value k means ids[k - 1] owns that pixel -- 1-based
    so 0 stays free for "no owner" (a valid, meaningful label, not just an
    unset default). uint16 (not uint8) because seam_masks.keys() is not
    inherently bounded to 255 images."""
    canvas_w, canvas_h = canvas_size
    ids = list(seam_masks.keys())
    ...
    for idx, image_id in enumerate(ids, start=1):
        ...
        if x1 <= x0 or y1 <= y0:
            continue
        local_mask = seam_masks[image_id][y0 - y : y1 - y, x0 - x : x1 - x]
        owner_map[y0:y1, x0:x1][local_mask > 0] = idx
