...
class MosaicResult:
    ...
    # Persisted by pipeline/runner.py as {facade_id}_homographies.json /
    # _seam_owner_map.png / _seam_owner_index.json -- crack/pipeline.py reads
    # these back (in a completely separate later run) to compute each crack's
    # source_observations. source_transforms maps image_id -> SourceTransform
    # (H: source-pixel -> canvas-pixel, + that source image's own size).
    source_transforms: dict[str, SourceTransform] = None
    seam_owner_map: np.ndarray | None = None  # uint16, 0 = no owner, k = seam_owner_index[k-1]
    seam_owner_index: list = None
