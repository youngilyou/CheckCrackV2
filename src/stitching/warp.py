...
class SourceTransform:
    """Original image pixel -> final composited canvas pixel, plus that
    original image's own (width, height) -- everything crack/pipeline.py
    needs to invert a mosaic-space crack location back into one specific
    source image's own pixel coordinates and clip it to that image's bounds
    (see source_observations). `size` here is the SOURCE image's size, not
    `WarpedImage.size` (which is the warped/output ROI or canvas size)."""

    H: np.ndarray  # 3x3, source-image-pixel -> canvas-pixel
    width: int
    height: int
...
def warp_images(
    ...
    per-image SourceTransform). SourceTransform.H maps ORIGINAL image pixel ->
    FINAL composited canvas pixel (i.e. `global_shift @ H`, not the
    `homographies` input, and not `H_local` either -- `H_local` only maps into
    this image's own small ROI raster, not the shared canvas coordinate space
    that crack polygons/bboxes are measured in). Its inverse is exactly what a
    caller needs to map a mosaic-space crack location back to this source
    image's own pixel coordinates (see crack/pipeline.py's
    source_observations).
    """
    ...
    for image_id, H in homographies.items():
        ...
        source_transforms[image_id] = SourceTransform(H=H_global, width=w, height=h)
